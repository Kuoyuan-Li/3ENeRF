...
class BaseSystem(pl.LightningModule, SaverMixin):
    ...
    """
    def __init__(self, config): # config is the full config
        super().__init__()
        self.config = config
        self.rank = get_rank() # device index
        print(f'system rank: {self.rank}')
        self.prepare() # initialize criterions, train_num_samples, train_num_rays
        self.model = models.make(self.config.model.name, self.config.model) # initialise model (nerf)
        for n, p in self.model.named_parameters():
            print(f'{n} requires grad: {p.requires_grad}')
            print(f'{n} is_leaf: {p.is_leaf}')
    ...
    def on_train_batch_start(self, batch, batch_idx, unused=0):
        # print("on_train_batch_start called")
    ...
    def configure_optimizers(self):
        ...
        # called at the beginning of fit() (only once)

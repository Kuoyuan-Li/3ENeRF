...
class MarchingCubeHelper(nn.Module):
    def __init__(self, resolution, use_torch=True):
        ...
        self.use_torch = use_torch # false
...
class BaseImplicitGeometry(BaseModel):
    def __init__(self, config):
        super().__init__(config) # geometry config
        ...
        # self.contraction_type = None # assigned in main model
    ...
    @torch.no_grad()
    def isosurface(self): # for export mesh
...
class VolumeDensity(BaseImplicitGeometry): # used by nerf
    def setup(self):
        self.n_input_dims = self.config.get('n_input_dims', 3) # xyz
        self.n_output_dims = self.config.feature_dim # feature dim - 16 for nerf
        # get multi-resolution HashGrid
    ...
    def forward(self, points):
        # points = contract_to_unisphere(points.clone(), self.radius, self.contraction_type)
        ...
        if 'density_activation' in self.config: # truncted exp

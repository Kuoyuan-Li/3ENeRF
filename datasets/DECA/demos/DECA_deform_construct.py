...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            opdict, visdict = deca.decode(codedict) # decode cropped image
            if args.render_orig: # true
                tform = testdata[i]['tform'][None, ...] # 2D similarity transform
        ...
        if args.saveNeededOnly:
            ...
            # np.savetxt(os.path.join(savefolder, name, name + '_kpt2d_crop.txt'), opdict['landmarks2d'][0].cpu().numpy())
            # np.savetxt(os.path.join(savefolder, name, name + '_kpt3d_crop.txt'), opdict['landmarks3d'][0].cpu().numpy())
    ...
    with open(os.path.join(savefolder, 'exp_pose_dict.json'), 'w') as f:

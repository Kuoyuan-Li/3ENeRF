...
class DECA(nn.Module):
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        ## decode, verts is vertex
        # tune shape, expression and pose parameters to make the landmarks fit the image
        ...
        ## projection onto (cropped) images
        ...
        if return_vis and render_orig and original_image is not None and tform is not None:
            ...
            trans_verts = transform_points(trans_verts, tform, points_scale, [h, w]) # this step scale the x and y but not z
        ...
        if return_vis:
            ...
            # upsample mesh to create dense verts
            i = 0
            ...
            # expand dense_vertices on first dimension
            dense_vertices = dense_vertices[np.newaxis,...]
            # convert dense_vertices to tensor
            dense_vertices = torch.from_numpy(dense_vertices).float().to(self.device)
            ...
            if return_vis and render_orig and original_image is not None and tform is not None:
                ...
                _, _, h, w = original_image.shape
                # import ipdb; ipdb.set_trace()
                trans_dense_verts = transform_points(trans_dense_verts, tform, points_scale, [h, w]) # this step scale the x and y but not z
                opdict['trans_dense_verts'] = trans_dense_verts

...
def transform_points(points, tform, points_scale=None, out_scale=None):
    ...
    if points.shape[-1] == 3: # verts
        points_z = points[:,:,2:]
        scale_x = torch.sqrt(tform[:,0,0]**2 + tform[:,0,1]**2)
        if points_scale:
            points_z = (points_z*0.5 + 0.5)*points_scale[0] 
        points_z =  points_z*float(scale_x)
        if out_scale: # h,w of output image size
            points_z = points_z[:,:,0]/out_scale[0]*2 - 1
            points_z = torch.unsqueeze(points_z, dim=-1)
        trans_points = torch.cat([trans_points_2d[:,:,:2], points_z[:,:,:1]], dim=-1)
    else:
        trans_points = torch.cat([trans_points_2d[:,:,:2], points[:,:,2:]], dim=-1)
    return trans_points